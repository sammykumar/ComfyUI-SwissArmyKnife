...
class TemplateService {
    ...
    async loadTemplate(name) {
        try {
            ...
            const template = templates.find(
                (t) => t && (t.name === name || t.id === name || t.title === name || t === name)
            );
            ...
            const fetchByName = async () => {
                ...
                try {
                    let resp = await fetch(
                        `/super_lora/templates?name=${encodeURIComponent(name)}`
                    );
                    if (resp.ok) {
            ...
            if (!template) {
                console.warn(
                    `Super LoRA Loader: Template "${name}" not found in cache, trying GET by name`
                );
            ...
            const normalize = (cfg) => ({
                ...
                enabled:
                    cfg.enabled !== void 0 ? !!cfg.enabled : cfg.on === void 0 ? true : !!cfg.on,
                strength_model:
                    cfg.strength_model !== void 0
                        ? Number(cfg.strength_model)
                        : Number(cfg.strength ?? cfg.value ?? 1),
                strength_clip:
                    cfg.strength_clip !== void 0
                        ? Number(cfg.strength_clip)
                        : Number(
                              cfg.strengthTwo ??
                                  cfg.clip_strength ??
                                  cfg.strength_model ??
            ...
            if (validConfigs.length !== list.length) {
                console.warn(
                    `Super LoRA Loader: Some LoRA configs in template "${name}" are invalid`
                );
            }
            console.log(
                `Super LoRA Loader: Loaded template "${name}" with ${validConfigs.length} LoRAs`
            );
            return validConfigs;
        } catch (error) {
    ...
    async deleteTemplate(name) {
        try {
            ...
            if (response.ok) {
                await this.loadTemplates();
                const stillExists = (this.templates || []).some((t) =>
                    t && t.name ? t.name === name : t === name
                );
                if (stillExists) {
                    console.warn(
                        `Super LoRA Loader: Server responded OK but template still present: ${name}`
                    );
                    return false;
                }
...
class CivitAiService {
    ...
    async getTriggerWords(loraFileName) {
        try {
            ...
            console.warn(
                `Super LoRA Loader: Failed to get trigger words for ${loraFileName}:`,
                error
            );
            return [];
        }
    ...
    async fetchModelInfo(loraFileName) {
        try {
            ...
            if (data.error) {
                console.warn(
                    `Super LoRA Loader: CivitAI API error for ${loraFileName}:`,
                    data.error
                );
                return null;
            }
            ...
            console.warn(
                `Super LoRA Loader: Failed to fetch model info for ${loraFileName}:`,
                error
            );
            return null;
        }
...
class OverlayService {
    ...
    `;
        ...
        list.style.cssText =
            "display: flex; flex-direction: column; gap: 4px; padding: 0 8px 8px 8px;";
        ...
        const render = (term) => {
            ...
            filtered.slice(0, maxToShow).forEach((item) => {
                ...
                if (options.rightActions && options.rightActions.length) {
                    ...
                    actions.push({
                        icon: options.rightActionIcon || "🗑",
                        title: options.rightActionTitle,
                        onClick: options.onRightAction,
                    });
                }
    ...
    `;
        ...
        clearBtn.style.cssText =
            "padding: 8px 12px; border-radius: 6px; background: #333; color: #fff; border: 1px solid #555; cursor: pointer;";
        ...
        cancelBtn.style.cssText =
            "padding: 8px 12px; border-radius: 6px; background: #444; color: #fff; border: 1px solid #555; cursor: pointer;";
        cancelBtn.addEventListener("click", () => closeOverlay());
        const addBtn = document.createElement("button");
        addBtn.type = "button";
        addBtn.textContent = "Add Selected (0)";
        addBtn.style.cssText =
            "padding: 8px 12px; border-radius: 6px; background: #1976d2; color: #fff; border: 1px solid #0d47a1; cursor: pointer; opacity: 0.6;";
        ...
        const allowFolderChips =
            (Array.isArray(options.folderChips) && options.folderChips.length > 0) ||
    ...
    persistOverlayState(baseFolderKey, folders, subfolders) {
        try {
            sessionStorage.setItem(
                this.getFolderStorageKey(baseFolderKey),
                JSON.stringify(Array.from(folders))
            );
            sessionStorage.setItem(
                this.getSubfolderStorageKey(baseFolderKey),
                JSON.stringify(Array.from(subfolders))
            );
        } catch {}
    }
    ...
    /**
     * Show dual-panel LoRA selector for high noise and low noise models
     */
    showDualLoraSelector(options) {
        this.ensureOverlayStyles();
        this.removeExistingOverlays();

        const items = options.items ?? [];
        const usedLoras = options.usedLoras ?? new Set();
    ...
    `;
        ...
        // Create header
        const header = document.createElement("div");
        header.textContent = options.title || "Add LoRA Pair";
        header.style.cssText = `
      padding: 12px 14px;
      font-weight: 600;
      border-bottom: 1px solid #444;
    ...
    `;
        ...
        // Shared state
        let selectedHighNoise = null;
        let selectedLowNoise = null;
        ...
        // Create left panel (High Noise)
        const leftPanel = this.createLoraPanel({
            title: "High Noise LoRA",
            accentColor: "#1976d2",
            items,
            usedLoras,
            onSelect: (id) => {
                selectedHighNoise = id;
                updateFooter();
            },
        });

        // Create right panel (Low Noise)
        const rightPanel = this.createLoraPanel({
    ...
    `;
        ...
        statusText.style.cssText = "color: #aaa; font-size: 13px;";
        statusText.textContent = "Select LoRAs from both panels";
    ...
    `;
        ...
        const updateFooter = () => {
            const hasHigh = !!selectedHighNoise;
            const hasLow = !!selectedLowNoise;

            if (hasHigh && hasLow) {
                statusText.textContent = `Ready to add: ${selectedHighNoise} (high) + ${selectedLowNoise} (low)`;
                addBtn.disabled = false;
                addBtn.style.opacity = "1";
            } else if (hasHigh) {
                statusText.textContent = `High noise selected: ${selectedHighNoise}. Select low noise LoRA.`;
                addBtn.disabled = true;
                addBtn.style.opacity = "0.5";
            } else if (hasLow) {
                statusText.textContent = `Low noise selected: ${selectedLowNoise}. Select high noise LoRA.`;
                addBtn.disabled = true;
                addBtn.style.opacity = "0.5";
            } else {
                statusText.textContent = "Select LoRAs from both panels";
        ...
        addBtn.addEventListener("click", () => {
            if (selectedHighNoise && selectedLowNoise && options.onAddPair) {
                options.onAddPair(selectedHighNoise, selectedLowNoise);
                closeOverlay();
            }
        });
    ...
    /**
     * Create a single LoRA selection panel for dual-panel view
     */
    createLoraPanel(options) {
        const panel = document.createElement("div");
        panel.style.cssText = `
      flex: 1;
      background: #222;
    ...
    `;
        ...
        panelHeader.textContent = options.title;
    ...
    `;
        ...
        // List container
        const listWrap = document.createElement("div");
        listWrap.style.cssText = "overflow: auto; padding: 6px 4px 10px 4px; flex: 1;";
        ...
        const empty = document.createElement("div");
        empty.textContent = "No results";
        empty.style.cssText = "padding: 12px; color: #aaa; display: none; text-align: center;";
        ...
        const render = (term) => {
            ...
            filtered.slice(0, 500).forEach((item) => {
                ...
                row.type = "button";
                row.className = "nd-overlay-item-button";
                row.disabled = options.usedLoras.has(item.id);
                row.style.cssText = `
          display: flex;
          align-items: center;
          gap: 8px;
          padding: 10px 12px;
          background: ${selectedId === item.id ? "rgba(255,255,255,0.1)" : "#2a2a2a"};
          border: 1px solid ${selectedId === item.id ? options.accentColor : "#3a3a3a"};
          border-radius: 6px;
          color: #fff;
        ...
        `;
                ...
                if (item.disabled || options.usedLoras.has(item.id)) {
                    const badge = document.createElement("span");
                    badge.textContent = "added";
                    badge.style.cssText =
                        "font-size: 10px; padding: 2px 6px; background: #555; border-radius: 4px; color: #aaa;";
                ...
                row.addEventListener("mouseenter", () => {
                    if (!row.disabled && selectedId !== item.id) {
...
const _FilePickerService = class _FilePickerService {
    ...
    tryHookComfyRefreshFunctions(attempt = 0) {
        const hooked =
            this.patchRefreshFunctions(window?.app, "app") ||
            this.patchRefreshFunctions(window?.api, "api") ||
            this.patchRefreshFunctions(window?.ui, "ui");
        if (hooked || attempt >= 20) {
            return;
        }
        setTimeout(
            () => this.tryHookComfyRefreshFunctions(attempt + 1),
    ...
    onExternalRefresh(trigger) {
        ...
        try {
            window.dispatchEvent(
                new CustomEvent("nd-super-nodes:files-refreshed", { detail: { trigger } })
            );
        } catch (error) {}
    }
    triggerLinkedServiceRefresh() {
        try {
            ...
            this.pendingLoraRefresh = Promise.resolve(loraService.refreshLoraList())
                .catch((error) => {
                    console.warn(
                        "ND Super Nodes: Failed to refresh LoRA list after global refresh",
                        error
                    );
                })
                .finally(() => {
                ...
                data.files?.map((file) => {
                    ...
                    return {
                        ...
                        label: file.name.replace(
                            /\.(ckpt|pt|pt2|bin|pth|safetensors|pkl|sft|gguf)$/i,
                            ""
                        ),
                        path: relativePath,
                        fullPath: file.path,
        ...
        this.getFilesForType(fileType)
            .then((files) => {
                ...
                overlay.showSearchOverlay({
                    title,
                    placeholder:
                        config.placeholder || `Search ${config.displayName.toLowerCase()}...`,
                    items,
                    allowCreate: false,
            ...
            .catch((error) => {
                ...
                OverlayService.getInstance().showToast(
                    `Failed to load ${config.displayName.toLowerCase()}`,
                    "error"
                );
            });
    }
    ...
    handleHitAreas(event, pos, node, handler) {
        ...
        for (const [key, area] of entries) {
            ...
            if (bounds && bounds.length >= 4 && this.isInBounds(pos, bounds)) {
                const fn =
                    (handler === "onDown" ? area.onDown : area.onClick) ||
                    (handler === "onDown" ? area.onClick : area.onDown);
                if (fn) {
...
class SuperLoraWidget extends SuperLoraBaseWidget {
    constructor(name) {
        ...
        this.onStrengthClipClick = (event, _pos, node) => {
            try {
                ...
                if (canvas?.prompt) {
                    canvas.prompt(
                        "CLIP Strength",
                        this.value.strengthClip ?? this.value.strength ?? 1,
                        (v) => {
                            const val = parseFloat(v);
                            if (!Number.isNaN(val)) {
        ...
        this.onStrengthClipDownClick = (_event, _pos, node) => {
            this.value.strengthClip = Math.max(
                -10,
                (this.value.strengthClip ?? this.value.strength ?? 1) - 0.1
            );
            node.setDirtyCanvas(true, false);
            try {
                WidgetAPI.syncExecutionWidgets(node);
        ...
        this.onStrengthClipUpClick = (_event, _pos, node) => {
            this.value.strengthClip = Math.min(
                10,
                (this.value.strengthClip ?? this.value.strength ?? 1) + 0.1
            );
            node.setDirtyCanvas(true, false);
        ...
        this.hitAreas = {
            ...
            strengthClipDown: {
                bounds: [0, 0],
                onClick: this.onStrengthClipDownClick,
                priority: 90,
            },
    ...
    draw(ctx, node, w, posY, height) {
        ...
        ctx.textBaseline = "middle";
        const topPad = node.properties?.showTriggerWords
            ? 4
            : Math.max(4, Math.floor((height - rowHeight) / 2));
    ...
    drawFirstRow(ctx, node, w, posY, rowHeight, fullHeight) {
        ...
        if (showMoveArrows) {
            const leftMostMinus =
                showStrength && node?.properties?.showSeparateStrengths
        ...
        const loraDisplay = this.truncateText(ctx, loraText, nameWidth - 30); // Leave space for badge
        ctx.fillText(loraDisplay, loraLeft, posY + midY);

        // Draw model type badge (high/low/both)
        if (this.value.modelType && this.value.modelType !== "both") {
            const badgeText = this.value.modelType === "high" ? "H" : "L";
            const badgeColor = this.value.modelType === "high" ? "#1976d2" : "#7b1fa2";
            const badgeSize = 18;
            const badgeX = loraLeft + ctx.measureText(loraDisplay).width + 6;
            const badgeY = posY + Math.floor((rowHeight - badgeSize) / 2);
            ...
            ctx.fillText(badgeText, badgeX + badgeSize / 2, posY + midY);
        }
        ...
        if (showTriggers && trigWidth > 0) {
            const hasTrigger = !!(
                this.value.triggerWords && String(this.value.triggerWords).trim()
            );
            const pillH = 20;
            ...
            if (hasTrigger) {
                ...
                const trigDisplay = this.truncateText(
                    ctx,
                    String(this.value.triggerWords),
                    trigWidth - padX * 2
                );
        ...
        if (showMoveArrows && node?.properties?.showMoveArrows !== false) {
            ...
            let disableUp;
            if (node?.properties?.enableTags) {
                const groupWidgets = (node.customWidgets || []).filter(
                    (w2) => w2 instanceof SuperLoraWidget && w2.value?.tag === this.value.tag
                );
            ...
            this.hitAreas.moveDown.bounds = disableDown
                ? [0, 0, 0, 0]
                : [downX, 0, arrowSize, fullHeight];
            ...
            this.hitAreas.moveUp.bounds = disableUp
                ? [0, 0, 0, 0]
                : [upX, 0, arrowSize, fullHeight];
            ctx.globalAlpha = controlsAlpha;
        } else {
        ...
        if (node?.properties?.showStrengthControls !== false) {
            ...
            ctx.font = "12px Arial";
            ctx.fillText(
                this.value.strength.toFixed(2),
                strengthX + strengthWidth / 2,
                posY + midY
            );
        ...
        if (showStrength && node?.properties?.showSeparateStrengths) {
            ...
            ctx.font = "12px Arial";
            ctx.fillText(
                this.value.strengthClip.toFixed(2),
                strengthClipX + strengthWidth / 2,
                posY + midY
            );
            ...
            WidgetAPI.showNameOverlay({
                ...
                onCommit: async (templateName) => {
                    ...
                    try {
                        ...
                        if (exists) {
                            WidgetAPI.showToast(
                                `⚠️ Template "${name}" already exists. Choose a different name.`,
                                "warning"
                            );
                            return;
                        }
                        const success = await WidgetAPI.templateService.saveTemplate(
                            name,
                            validConfigs
                        );
                        WidgetAPI.showToast(
                        ...
                        WidgetAPI.showToast(
                            "❌ Error saving template. Check console for details.",
                            "error"
                        );
                    }
                },
        ...
        if (totalWidth > availableForButtons) {
            ...
            buttons.forEach((btn, idx) => {
                ...
                if (mode === "icon") {
                    ...
                    label =
                        btn.combineIcon && btn.icon
                            ? `${btn.icon} ${btn.shortText}`
                            : btn.shortText;
                } else {
...
const _SuperLoraNode = class _SuperLoraNode {
        ...
        this.initializationPromise = (async () => {
            ...
            setWidgetAPI({
                showLoraSelector: (node, widget, e) =>
                    _SuperLoraNode.showLoraSelector(node, widget, e),
                ...
                showInlineText: (e, initial, onCommit, place) =>
                    _SuperLoraNode.showInlineText(e, initial, onCommit, place),
                ...
                if (!hasAddLoRA || !hasSettings) {
                    ...
                    if (!hasAddLoRA) {
                        options.push({
                            content: "🏷️ Add LoRA",
                            callback: (_event) =>
                                _SuperLoraNode.showLoraSelector(this, void 0, void 0),
                        });
                    }
                    if (!hasSettings) {
                        options.push({
    ...
    static setupAdvancedNode(node) {
        ...
        node.properties.useSplitView = node.properties.useSplitView !== false; // Enable split view by default
    ...
    static computeContentHeight(node) {
        ...
        // Check if using split view
        const useSplitView = node.properties?.useSplitView !== false;
        const loraWidgets = node.customWidgets.filter((w) => w instanceof SuperLoraWidget);
        const hasModelTypes = loraWidgets.some(
            (w) => w.value?.modelType === "high" || w.value?.modelType === "low"
        );

        if (useSplitView && hasModelTypes) {
            // Calculate height for split view layout
            const headerWidgets = node.customWidgets.filter(
                (w) => w instanceof SuperLoraHeaderWidget || w instanceof SuperLoraTagWidget
            );
            ...
            for (const widget of headerWidgets) {
                ...
                const marginAfter =
                    widget instanceof SuperLoraTagWidget && widget.isCollapsed()
                        ? 0
                        : marginDefault;
                currentY += size[1] + marginAfter;
            }

            // Add column header height if we have high/low loras
            const highLoras = loraWidgets.filter((w) => w.value?.modelType === "high");
            const lowLoras = loraWidgets.filter((w) => w.value?.modelType === "low");
            const bothLoras = loraWidgets.filter(
                (w) => !w.value?.modelType || w.value?.modelType === "both"
            );

            if (highLoras.length > 0 || lowLoras.length > 0) {
                currentY += 24 + marginDefault; // Column headers

                // Height for rows (max of high/low)
                const maxLoras = Math.max(highLoras.length, lowLoras.length);
                const loraHeight = 34;
                currentY += maxLoras * (loraHeight + marginDefault);
            }

            // Add 'both' loras height
            if (bothLoras.length > 0) {
                currentY += marginDefault; // Extra space before 'both' section
                currentY += bothLoras.length * (34 + marginDefault);
            }

            return Math.max(currentY, 100);
        }

        // Traditional single column height calculation
        const renderable = [];
        ...
        renderable.forEach((widget, index) => {
            ...
            const height = widget instanceof SuperLoraWidget ? 34 : size[1];
            let marginAfter =
                widget instanceof SuperLoraTagWidget && widget.isCollapsed() ? 0 : marginDefault;
    ...
    static drawCustomWidgets(node, ctx) {
        ...
        if (!isBypassed) {
            ...
            if (useSplitView && hasModelTypes) {
                // Split view mode: draw high/low in columns
                currentY = this.drawSplitView(node, ctx, currentY, marginDefault);
            } else {
                // Traditional single column mode
                currentY = this.drawSingleColumn(node, ctx, currentY, marginDefault);
            }
        }
            ...
            try {
                ...
                if (typeof ctx.roundRect === "function") {
                    ...
                    ctx.roundRect(
                        4,
                        4,
                        Math.max(0, node.size[0] - 8),
                        Math.max(0, node.size[1] - 8),
                        radius
                    );
                    ctx.fill();
                } else {
    ...
    /**
     * Draw widgets in traditional single column
     */
    static drawSingleColumn(node, ctx, startY, marginDefault) {
        let currentY = startY;
        const renderable = [];
    ...
    /**
     * Draw widgets in split-screen mode (high on left, low on right)
     */
    static drawSplitView(node, ctx, startY, marginDefault) {
        let currentY = startY;
        const nodeWidth = node.size[0];
        const columnWidth = Math.floor(nodeWidth / 2) - 4; // Leave 4px gap in middle
        const leftX = 0;
        const rightX = Math.floor(nodeWidth / 2) + 4;

        // Draw header and tag widgets in full width
        const headerWidgets = node.customWidgets.filter(
            (w) => w instanceof SuperLoraHeaderWidget || w instanceof SuperLoraTagWidget
        );
        ...
        for (const widget of headerWidgets) {
            const size = widget.computeSize();
            if (size[1] === 0) continue;
            widget.draw(ctx, node, nodeWidth, currentY, size[1]);
            const marginAfter =
                widget instanceof SuperLoraTagWidget && widget.isCollapsed() ? 0 : marginDefault;
            currentY += size[1] + marginAfter;
        }

        // Separate LoRAs by modelType
        const loraWidgets = node.customWidgets.filter((w) => w instanceof SuperLoraWidget);
        const highLoras = loraWidgets.filter((w) => w.value?.modelType === "high");
        const lowLoras = loraWidgets.filter((w) => w.value?.modelType === "low");
        const bothLoras = loraWidgets.filter(
        ...
        // Draw section headers if we have high/low loras
        if (highLoras.length > 0 || lowLoras.length > 0) {
            const headerHeight = 24;

            // Draw "High Noise" header on left
            ctx.save();
            ctx.fillStyle = "#1976d2";
            ctx.beginPath();
            ctx.roundRect(8, currentY, columnWidth - 8, headerHeight, 4);
            ctx.fill();
            ctx.fillStyle = "#fff";
            ctx.font = "bold 11px 'Segoe UI', Arial, sans-serif";
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText("High Noise", 8 + (columnWidth - 8) / 2, currentY + headerHeight / 2);

            // Draw "Low Noise" header on right
            ctx.fillStyle = "#7b1fa2";
            ctx.beginPath();
            ctx.roundRect(rightX, currentY, columnWidth - 8, headerHeight, 4);
            ctx.fill();
            ctx.fillStyle = "#fff";
            ctx.fillText("Low Noise", rightX + (columnWidth - 8) / 2, currentY + headerHeight / 2);
            ctx.restore();

            currentY += headerHeight + marginDefault;
        }

        // Draw high and low loras in columns
        const maxLoras = Math.max(highLoras.length, lowLoras.length);
        const loraHeight = 34;
        ...
        for (let i = 0; i < maxLoras; i++) {
            ...
            // Draw high noise lora on left
            if (i < highLoras.length) {
                const widget = highLoras[i];
                ctx.save();
                ctx.translate(leftX, 0);
                ctx.beginPath();
                ctx.rect(8, rowY, columnWidth - 8, loraHeight);
                ctx.clip();
                widget.draw(ctx, node, columnWidth, rowY, loraHeight);
                ctx.restore();

                // Store position for hit detection
                widget._splitViewBounds = {
                    x: leftX + 8,
                    y: rowY,
                    width: columnWidth - 8,
                    height: loraHeight,
            ...
            // Draw low noise lora on right
            if (i < lowLoras.length) {
                const widget = lowLoras[i];
                ctx.save();
                ctx.translate(rightX, 0);
                ctx.beginPath();
                ctx.rect(0, rowY, columnWidth - 8, loraHeight);
                ctx.clip();
                widget.draw(ctx, node, columnWidth, rowY, loraHeight);
                ctx.restore();
            ...
            currentY += loraHeight + marginDefault;
        }

        // Draw 'both' loras in full width at the bottom
        if (bothLoras.length > 0) {
            currentY += marginDefault; // Extra space before 'both' section

            for (const widget of bothLoras) {
                widget.draw(ctx, node, nodeWidth, currentY, loraHeight);
                widget._splitViewBounds = null; // Clear split view bounds
                currentY += loraHeight + marginDefault;
            }
        }
    ...
    static handleMouseEvent(node, event, pos, handler) {
        ...
        // Check for split view widgets first
        for (const widget of node.customWidgets) {
            if (widget._splitViewBounds) {
                const bounds = widget._splitViewBounds;
                if (
                    pos[0] >= bounds.x &&
                    ...
                    const localPos = [pos[0] - bounds.x, pos[1] - bounds.y];
                    if (widget[handler]) {
        ...
        // Fall back to traditional single column hit detection
        const marginDefault = _SuperLoraNode.MARGIN_SMALL;
        let currentY = this.NODE_WIDGET_TOP_OFFSET;
        for (const widget of node.customWidgets) {
            // Skip widgets with split view bounds (already checked above)
            if (widget._splitViewBounds) continue;
    ...
    static computeWidgetTop(node, targetWidget) {
        ...
        for (const widget of node.customWidgets) {
            ...
            const isCollapsed =
                widget instanceof SuperLoraWidget && widget.isCollapsedByTag?.(node);
            const height = widget instanceof SuperLoraWidget ? 34 : size[1];
        ...
        try {
            ...
            if (!widget) {
                OverlayService.getInstance().showDualLoraSelector({
                    ...
                    items,
                    usedLoras,
                    onAddPair: (highNoiseId, lowNoiseId) => {
                        // Add high noise LoRA
                        this.addLoraWidget(node, {
                            lora: highNoiseId,
                            modelType: "high",
                        });
                        ...
                        this.showToast("✅ LoRA pair added (High + Low)", "success");
                        node.setDirtyCanvas(true, true);
                    },
                });
                return;
            ...
            // Single LoRA selection when editing existing widget
            OverlayService.getInstance().showSearchOverlay({
                ...
                onChooseMany: (ids) => {
                    ...
                    if (added.length)
                        this.showToast(
                            `✅ Added ${added.length} LoRA${added.length > 1 ? "s" : ""}`,
                            "success"
                        );
                    if (skipped.length)
                        this.showToast(
                ...
                folderChips: Array.from(
                    new Set(availableLoras.map((p) => p.split(/[\\/]/)[0]).filter(Boolean))
                ).sort(),
    ...
    static showTagSelector(node, widget) {
        ...
        const allTags = Array.from(/* @__PURE__ */ new Set([...fromStore, ...existingTags]));
                    ...
                    onClick: (name) => {
                        this.showNameOverlay({
                            ...
                            onCommit: (newName) => {
                                ...
                                this.showToast(
                                    ok ? "✅ Tag renamed" : "❌ Failed to rename tag",
                                    ok ? "success" : "error"
                                );
    ...
    static showSettingsDialog(node, event) {
        const coreItems = [
            ...
            },
            {
                ...
                callback: () => {
                    ...
                    try {
                        const widgets = (node.customWidgets || []).filter(
                            (w) => w instanceof SuperLoraWidget
                        );
                        if (enabling) {
                            widgets.forEach((w) => {
                                const m = parseFloat(w.value?.strength ?? 0) || 0;
                                w.value.strengthClip =
                                    typeof w.value?.strengthClip === "number"
                                        ? w.value.strengthClip
                                        : m;
                            });
                        } else {
        ...
        const showItems = [
            ...
            },
            {
                ...
                callback: () => {
                    node.properties.showTagChip =
                        node.properties.showTagChip === false ? true : false;
                    node.setDirtyCanvas(true, false);
                    this.syncExecutionWidgets(node);
                },
            },
    ...
    static showSaveTemplateDialog(node) {
        ...
        if (templateName && templateName.trim()) {
            ...
            this.templateService.saveTemplate(templateName.trim(), configs).then((success) => {
                if (success) {
                    this.showToast(
                        `Template "${templateName.trim()}" saved successfully!`,
                        "success"
                    );
                } else {
                ...
                onChoose: async (name) => {
                    try {
                        ...
                        if (template) {
                            ...
                            this.showToast(
                                `❌ Failed to load template "${name}". It may be corrupted.`,
                                "error"
                            );
                        }
                    } catch (error) {
                        ...
                        this.showToast(
                            `❌ Error loading template. Check console for details.`,
                            "error"
                        );
                    }
                },
                rightActions: [
                        ...
                        onClick: async (name) => {
                            this.showNameOverlay({
                                ...
                                onCommit: async (newName) => {
                                    ...
                                    this.showToast(
                                        ok ? "✅ Template renamed" : "❌ Failed to rename",
                                        ok ? "success" : "error"
                                    );
                    ...
                    },
                    {
                        ...
                        onClick: async (name) => {
                            ...
                            this.showToast(
                                deleted ? "✅ Template deleted" : "❌ Failed to delete template",
    ...
    static organizeByTags(node) {
        if (!node.properties.enableTags) {
            node.customWidgets = node.customWidgets.filter(
                (w) => !(w instanceof SuperLoraTagWidget)
            );
            return;
        }
        const loraWidgets = node.customWidgets.filter((w) => w instanceof SuperLoraWidget);
    ...
    static getUsedLoras(node) {
        return new Set(
            node.customWidgets
                .filter((w) => w instanceof SuperLoraWidget)
                .map((w) => w.value.lora)
                .filter((lora) => lora && lora !== "None")
        );
    }
    /**
    ...
    static buildBundle(node) {
        ...
        const bundle = loraWidgets
            .filter((w) => w?.value?.lora && w.value.lora !== "None")
            .map((w) => ({
                lora: w.value.lora,
                enabled: w.value.enabled,
        ...
        try {
            ...
            if (Array.isArray(data.widgets)) {
                for (const widgetData of data.widgets) {
                    ...
                    widget.value = {
                        ...widget.value,
                        ...JSON.parse(JSON.stringify(widgetData.value || {})),
                    };
                    restoredWidgets.push(widget);
                }
            }
            node.customWidgets = restoredWidgets.length
                ? restoredWidgets
                : [new SuperLoraHeaderWidget()];
        } catch (error) {
    ...
    static showInlineNumber(event, initial, onCommit) {
        ...
        try {
            ...
            console.warn(
                "[showInlineNumber] Coordinate calculation failed, using fallback:",
                error
            );
            const lastPointer = _SuperLoraNode._lastPointerScreen;
            leftPx = (lastPointer?.x ?? 100) + 8;
        ...
        try {
            if (event && typeof event.clientX === "number" && typeof event.clientY === "number") {
                ...
                if (cRect) {
                    ...
                    console.log(
                        `[showInlineText] Using place rect: ${leftPx}, ${topPx}, rect:`,
                        rect
                    );
                } else {
...
const _NodeEnhancerExtension = class _NodeEnhancerExtension {
    ...
    static isAutoEnhanceEnabled() {
        try {
            ...
            return (
                app$1?.ui?.settings?.getSettingValue?.(this.SETTINGS.autoEnhanceAll, false) === true
            );
        } catch {
            return false;
        }
    ...
    static isGloballyEnabled() {
        try {
            ...
            return (
                app$1?.ui?.settings?.getSettingValue?.(this.SETTINGS.contextToggle, true) !== false
            );
        } catch {
            return true;
        }
    ...
    static createOverlayWidget(node, targetWidget, config) {
        const overlayWidget = {
            ...
            draw(ctx, nodeRef, widgetWidth, widgetY) {
                ...
                const w = Math.max(
                    120,
                    Math.min(availableWidth, (nodeRef?.size?.[0] || widgetWidth) - 24)
                );
                ...
                const rawValue =
                    (this._ndDisplayValue && String(this._ndDisplayValue)) || this._ndPlaceholder;
                ...
                const displayLabel = showLabel
                    ? _NodeEnhancerExtension.truncateText(ctx, labelText, labelWidth, false)
                    : "";
                const displayValue =
                    valueFullWidth <= valueMaxWidth
                        ? rawValue
                        : _NodeEnhancerExtension.truncateText(ctx, rawValue, valueMaxWidth, false);
                if (showLabel) {
                    ctx.textAlign = "left";
            ...
            mouse(event, pos, nodeRef) {
                ...
                const isLeft =
                    event?.button === 0 || event?.which === 1 || event?.button === void 0;
        ...
        overlayWidget.updateDisplay = (value, displayValue) => {
            ...
            const normalizedDisplay =
                typeof displayValue === "string" ? displayValue : normalizedValue;
    ...
    static ensureWidgetValueSelectable(widget, value) {
        ...
        try {
            ...
            if (!widget.options) {
                ...
                _NodeEnhancerExtension.debugLog("Initialized widget options for value", value, {
            ...
            if (Array.isArray(values)) {
                if (!values.includes(value)) {
                    ...
                    _NodeEnhancerExtension.debugLog("Appended value to array options", value, {
                        widgetName: widget.name,
                        before,
                        after: options.values,
                    });
                }
                return;
            }
            if (values && typeof values === "object") {
                if (!(value in values)) {
                    ...
                    _NodeEnhancerExtension.debugLog("Added value to object options", value, {
                        widgetName: widget.name,
                        before,
                        after: options.values,
                    });
            ...
            _NodeEnhancerExtension.debugLog("Reset options.values to single entry", value, {
                widgetName: widget.name,
                before,
            });
        } catch (error) {
    ...
    static enhanceWidget(node, widget, config) {
        ...
        if (!("options" in original))
            original.options = widget.options ? { ...widget.options } : void 0;
        if (!("values" in original))
            original.values = widget.options?.values ? [...widget.options.values] : void 0;
    ...
    static setup(nodeType, nodeData) {
        ...
        nodeType.prototype.getExtraMenuOptions = function (canvas, optionsArr) {
            ...
            const originalResult = originalMenu
                ? originalMenu.call(this, canvas, providedOptions)
                : providedOptions;
            ...
            const toggleOption = {
                content: this.__ndPowerEnabled
                    ? "➖ Disable ND Super Selector"
                    : "⚡ Enable ND Super Selector",
                callback: () => {
                    try {
                        if (this.__ndPowerEnabled) {
        ...
        nodeType.prototype.serialize = function () {
            ...
            try {
                ...
                const hasEnhancements =
                    configs.length > 0 ||
                    (this.__ndEnhancedWidgets && Object.keys(this.__ndEnhancedWidgets).length > 0);
                if (!hasEnhancements) {
                    return data;
                }
        ...
        nodeType.prototype.onMouseDown = function (event, pos) {
            ...
            const fallbackConfigs = activeConfigs.length
                ...
                : Object.keys(this.__ndEnhancedWidgets || {})
                      .map((widgetName) =>
                          _NodeEnhancerExtension.createConfigFromWidget(this, widgetName)
                      )
                      .filter((cfg) => !!cfg);
            const handled = fallbackConfigs.some((cfg) =>
                _NodeEnhancerExtension.handleEnhancedMouseDown(this, event, pos, cfg)
            );
            if (handled) {
                return true;
            }
        ...
        if (Array.isArray(baseConfigs) && baseConfigs.length) {
            console.log(
                `Node Enhancer: Registered ND Super Selector support for ${nodeData?.name}`
            );
        }
    }
    /**
    ...
    static setupEnhancedNode(node, config) {
        ...
        if (!widget) {
            console.warn(
                `Node Enhancer: Could not find widget "${config.widgetName}" in ${config.nodeType}`
            );
            return;
        }
        node.__ndPowerEnabled = true;
        node.__ndEnhancedWidgets = node.__ndEnhancedWidgets || {};
        node.__ndEnhancedWidgets[config.widgetName] = node.__ndEnhancedWidgets[
            config.widgetName
        ] || { original: {} };
        ...
        this.filePickerService.showFilePicker(
            ...
            (file) => {
                ...
                if (typeof newValue === "string") {
                    try {
                        const normalizedValue = _NodeEnhancerExtension.normalizeValueForWidget(
                            widget,
                            newValue
                        );
    ...
    static handleEnhancedMouseDown(node, event, pos, config) {
        if (
            !node.__ndPowerEnabled ||
            !node.__ndEnhancedWidgets ||
            !node.__ndEnhancedWidgets[config.widgetName]
        )
            return false;
        const widget = node.widgets?.find((w) => w.name === config.widgetName);
        ...
        try {
            const isLeft =
                event?.button === 0 || event?.which === 1 || !("button" in (event || {}));
    ...
    static handleSettingChanged(key, value) {
        ...
        if (
            key === this.SETTINGS.contextToggle ||
            key === this.SETTINGS.autoEnhanceAll ||
            key === this.SETTINGS.enabled
        ) {
    ...
    static findSettingElement(id) {
        ...
        try {
            const stored =
                settings?.settings?.get?.(id)?.element ?? settings?.settings?.[id]?.element;
    ...
    static getBaseConfigsForType(nodeTypeName) {
        ...
        return this.ENHANCED_NODES.filter((cfg) => cfg.nodeType === nodeTypeName).map((cfg) => ({
            ...cfg,
        }));
    }
    ...
    static createConfigFromWidget(node, widgetName) {
        ...
        const widget = Array.isArray(node.widgets)
            ? node.widgets.find((w) => w && w.name === widgetName)
            : null;
        let fileType = null;
    ...
    static guessFileTypeFromValues(values, widgetName, nodeType) {
        ...
        if (includes("/unet") || includes("\\unet") || includes("diffusion")) {
            return lowered.some((val) => val.endsWith(".gguf"))
                ? "gguf_unet_models"
                : "diffusion_models";
        }
        if (includes("upscale")) {
        ...
        for (const entry of lowered) {
            ...
            if (ext === ".gguf") {
                if (
                    candidates.includes("text_encoders") &&
                    (entry.includes("clip") ||
                        entry.includes("text") ||
                        widgetName.toLowerCase().includes("clip"))
                ) {
                    return "text_encoders";
                }
...
const nodeEnhancerExtension = {
    ...
    nodeCreated(node) {
        ...
        const autoEnhanceAll =
            enabled && app$1.ui.settings.getSettingValue("nodeEnhancer.autoEnhanceAll", false);
...
// Helper to check if a node type is supported
const isSupportedNodeType = (nodeType) => SUPPORTED_NODE_TYPES.includes(nodeType);
...
const superLoraExtension = {
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        if (isSupportedNodeType(nodeData.name)) {
            ...
            try {
                ...
                console.error(
                    "Super LoRA Loader: Error during node setup; continuing with default registration",
                    err
                );
            }
        }
