...
def register_restart_routes(app):
    ...
    async def restart_server(request):
        ...
        """
        try:
            logger.info("Swiss Army Knife: Server restart requested...")
            ...
            async def do_restart():
                ...
                logger.info("Swiss Army Knife: Restarting server...")
                ...
                logger.info("Swiss Army Knife: Force restarting...")
                sys.exit(0)
            ...
            logger.error(f"Swiss Army Knife: Error restarting server: {e}")
            return web.json_response({
                "success": False,
                "error": str(e)
            }, status=500)
    
    logger.info("Swiss Army Knife: Registered restart API route")
