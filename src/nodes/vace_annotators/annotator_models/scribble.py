"""
Vendored scribble generator helpers for VACE nodes (minimal port)
"""
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc: int, output_nc: int, ngf: int = 64, norm_type: str = "instance", use_dropout: bool = False, n_blocks: int = 9):
        assert n_blocks >= 0
        super().__init__()
        ...
        for _ in range(2):
            model += [nn.Conv2d(in_features, out_features, kernel_size=3, stride=2, padding=1, bias=False), norm_layer(out_features), activation]
        ...
        out_features = in_features // 2
        for _ in range(2):
            model += [nn.ConvTranspose2d(in_features, out_features, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False), norm_layer(out_features), activation]
            in_features = out_features
            out_features = in_features // 2
        model += [nn.ReflectionPad2d(3), nn.Conv2d(in_features, output_nc, kernel_size=7, padding=0), nn.Tanh()]
...
class ContourInference(nn.Module):
    def __init__(self, input_nc: int, output_nc: int, n_residual_blocks: int = 9, sigmoid: bool = True):
        ...
        self.model0 = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(input_nc, 64, kernel_size=7), norm_layer(64), nn.ReLU(inplace=True))
        model1 = []
        in_features = 64
        out_features = in_features * 2
        for _ in range(2):
            model1 += [nn.Conv2d(in_features, out_features, kernel_size=3, stride=2, padding=1), norm_layer(out_features), nn.ReLU(inplace=True)]
            in_features = out_features
            out_features = in_features * 2
        ...
        out_features = in_features // 2
        for _ in range(2):
            model3 += [nn.ConvTranspose2d(in_features, out_features, kernel_size=3, stride=2, padding=1, output_padding=1), norm_layer(out_features), nn.ReLU(inplace=True)]
            in_features = out_features
            out_features = in_features // 2
...
def _mark_backend_metadata(model: nn.Module, backend: str):
    if backend == "vendor":
        model._sak_input_normalization = "zero_to_one"
        model._sak_postprocess = "vendor"
    else:
        model._sak_input_normalization = "tanh"
        model._sak_postprocess = "quantile"
    model._sak_backend = backend
...
def postprocess_scribble_output(output: torch.Tensor, target_hw: Tuple[int, int], user_threshold: float = 0.12, apply_thinning: bool = True) -> torch.Tensor:
    if output.ndim == 3:
    ...
    if mag.shape[1] > 1:
        ...
        mag = mag.abs()
    q = 0.5
    thresh = _quantile_threshold(mag, q, user_threshold)
    mask = (mag > thresh).float()
    if apply_thinning:
        mask = _thin_edges(mask, iterations=2)
    mask = F.interpolate(mask, size=target_hw, mode='bilinear', align_corners=False)
    scribble_maps = 1.0 - mask.repeat(1, 3, 1, 1)
    return scribble_maps.permute(0, 2, 3, 1).cpu()


def postprocess_vendor_scribble_output(output: torch.Tensor, target_hw: Tuple[int, int]):
    if output.ndim == 3:
        output = output.unsqueeze(1)
    mask = torch.sigmoid(output)
    mask = F.interpolate(mask, size=target_hw, mode='bilinear', align_corners=False)
    mask = _thin_edges((mask > 0.5).float(), iterations=2)
    scribble_maps = 1.0 - mask.repeat(1, 3, 1, 1)
    return scribble_maps.permute(0, 2, 3, 1).cpu()
