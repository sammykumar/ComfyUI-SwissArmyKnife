...
def get_local_version() -> Dict[str, Any]:
    ...
    return {"version": version, "builtAt": built_at}
            ...
            async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
                async with session.get(GITHUB_RELEASE_URL) as response:
                    if response.status != 200:
                        ...
                        print("ND Super Nodes: GitHub release check failed", response.status, text[:200])
                        return None
...
def _compose_status(local_version: Dict[str, Any], release_payload: Optional[Dict[str, Any]], checked_at: datetime) -> Dict[str, Any]:
    ...
    return {
        ...
        "raw": {"tag": latest_tag, "name": (release_payload or {}).get("name"), "notes": (release_payload or {}).get("body"), "prerelease": (release_payload or {}).get("prerelease", False)},
    }
...
async def get_update_status(force: bool = False) -> Dict[str, Any]:
    ...
    async with _cache_lock:
        if (not force and _cache_data is not None and now - _cache_timestamp < CHECK_INTERVAL_SECONDS):
            return _cache_data
